...
<label><input type="checkbox" v-model="persist" /> 写入新记录</label>
<button @click="run">计算</button>
<p v-if="out">月供 {{ out.monthly_payment }} · 利息合计 {{ out.total_interest }}<template v-if="out.run_id"> · 已存为 #{{ out.run_id }}</template></p>
</div></template>
